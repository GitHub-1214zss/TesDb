  ...
  // 侧边栏点击隐藏兄弟元素
	...
	$('.x-slide_left').click(function(event) {
  	...
  	element.on('nav(side)', function(elem){
    	...
    	url = elem.find('cite').parent().attr('_href');
		...
		if(url!=null && url.length>0) {
			...
			element.tabChange('x-tab', tabCount);
	...
	//  	// alert(url);
	// 	 for (var i = 0; i <$('.x-iframe').length; i++) {
	// 	 	var aimURL = $('.x-iframe').eq(i).attr('src');
	// 	 	if(aimURL ==url){ element.tabChange('side',i); return;
	// 	 	}
	// 	 };
	// 	 res = element.tabAdd('x-tab', {
	// 	 	title: title//用于演示
	// 		 ,content: '<iframe frameborder="0" src="'+url+'" class="x-iframe"></iframe>' ,id : i });
	// 		 var tabIndex = $('.layui-tab-title li').length - 1;
	// 		 element.tabChange('x-tab',tabIndex);
	// 		 $('.layui-tab-title li').eq(0).find('i').remove();
	//  });
});
